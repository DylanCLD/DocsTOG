...
export function PageTreeNav({
  ...
  const roots = useMemo(() => buildHierarchy(pages, (page) => page.parent_page_id), [pages]);
  const initialOpenIds = useMemo(() => {
    if (defaultOpenAll) {
      const parentIds = new Set(pages.map((page) => page.parent_page_id).filter(Boolean) as string[]);
      return parentIds;
    }

    return activePageId ? collectAncestorIds(pages, activePageId, (page) => page.parent_page_id) : new Set<string>();
  }, [activePageId, defaultOpenAll, pages]);
